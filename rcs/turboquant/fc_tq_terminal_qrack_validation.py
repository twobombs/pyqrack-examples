...
def bench_qrack(width, depth, p, sdrp):
    ...
    for d in range(depth):
        ...
        exp_clone = experiment.clone()
        exp_clone.lossy_out_to_file("fc.svtq", p=p)
        exp_clone.lossy_in_from_file("fc.svtq")
...
def main():
    if len(sys.argv) < 3:
        raise RuntimeError(
            "Usage: python3 fc_tq_terminal_qrack_validation.py [width] [depth] [compression block size power] [sdrp]"
        )
