...
RCS_PROBE_PATCHES       = [0, 13, 22]  # one patch per worker - center patch 13 for routine validation
            ...
            for patch_id in assigned_patches:
                ...
                if do_rcs and patch_id in rcs_probe_this_step:
                    ...
                    try:
                        ...
                        # 1. Snapshot pristine statevector
                        ...
                        print(f"[Worker {rank}] RCS error (patch {patch_id}): {e}", file=sys.stderr)
                        rcs_xeb, rcs_hog = None, None
    ...
    def run(self, total_steps: int, dt: float, initial_hx: float,
        ...
        if RCS_VALIDATION_ENABLED:
            ...
            print(f"[Engine] Full-cube snapshots at steps: {RCS_FULL_SNAPSHOT_STEPS} "
                  f"(all 27 patches, DMA burst per patch, parallel across workers)")
